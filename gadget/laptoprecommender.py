...
train_matrix=pd.pivot_table(train_data,index='user_id',columns='laptop_id',values='ratings') 

#pivot table
final=pd.pivot_table(train_data,index='user_id',columns='laptop_id',values='rating_adjusted') #value with actual ratings
...
train_data = train_data.astype({"laptop_id": str})
gadget_user = train_data.groupby(by = 'user_id')['laptop_id'].apply(lambda x:','.join(x))
...
def User_item_score2(user):
    ...
    data = pd.DataFrame({'laptop_id':gadget_under_considiration,'score':score})
    top_recommendation=data.sort_values(by='score',ascending=False).head(4)
    gadget_name=top_recommendation.merge(Laptop, how='inner', on='laptop_id')
    gadget_id=gadget_name.laptop_id.values.tolist()
    return(gadget_id)

...
train_matrix=pd.pivot_table(train_data,index='user_id',columns='smartwatch_id',values='ratings') 

#pivot table
final=pd.pivot_table(train_data,index='user_id',columns='smartwatch_id',values='rating_adjusted') #value with actual ratings
...
train_data = train_data.astype({"smartwatch_id": str})
gadget_user = train_data.groupby(by = 'user_id')['smartwatch_id'].apply(lambda x:','.join(x))
...
def User_item_score2(user):
    ...
    data = pd.DataFrame({'smartwatch_id':gadget_under_considiration,'score':score})
    top_recommendation=data.sort_values(by='score',ascending=False).head(4)
    gadget_name=top_recommendation.merge(Smartwatch, how='inner', on='smartwatch_id')
    gadget_id=gadget_name.smartwatch_id.values.tolist()
    return(gadget_id)
